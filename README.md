...
Use the atlas summary and achievement panel to see visited-country coverage, saved cities, explorer XP, current level, and the next milestones.
...
- No sign-in, multi-device sync, collaboration, import, or export.
...
Hermes stores visited countries, visited cities, journal entries, and trips under the `hermes_travel_atlas_v1` browser-storage key. The application does not currently send that travel history to an application database.

Do not treat browser storage as a backup. Before adding accounts or cloud sync, define data export, deletion, retention, authentication, and recovery workflows.
